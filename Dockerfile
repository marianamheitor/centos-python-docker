...
RUN cd /tmp/ && curl https://ftp.openssl.org/source/old/1.1.1/openssl-1.1.1j.tar.gz -o openssl.tar.gz && \
    ...
    export LDFLAGS="-L/usr/lib64/openssl11" && export PYTHONPATH=/usr/local/bin/python3.12 && \
...
RUN cd /tmp/ && curl https://www.python.org/ftp/python/3.12.2/Python-3.12.2.tgz -o Python-3.12.2.tgz && \
    tar xzf Python-3.12.2.tgz && cd Python-3.12.2 && sed -i 's/PKG_CONFIG openssl /PKG_CONFIG openssl11 /g' configure && \
    ...
    cd .. && rm -rf Python-3.12.2.tgz && rm -rf Python-3.12.2
RUN ln -sfn /usr/local/bin/python3.12 /usr/bin/python3.12
...
RUN python3.12 -m pip install cmake
